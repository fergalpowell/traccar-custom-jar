...
reference:'menuNotificationsButton'},{text:Strings.sharedComputedAttributes,glyph:'xf0ae@FontAwesome',handler:'onComputedAttributesClick',reference:'menuComputedAttributesButton'},{text:Strings.sharedSavedCommands,glyph:'xf093@FontAwesome',handler:'onCommandsClick',reference:'menuCommandsButton'},{text:Strings.sharedMaintenances,glyph:'xf0ad@FontAwesome',handler:'onMaintenancesClick',reference:'menuMaintenancesButton'},{hidden:!0,text:Strings.sharedDeviceAccumulators,glyph:'xf0e4@FontAwesome',handler:'onDeviceAccumulatorsClick',
...
Ext.define('Traccar.view.map.BaseMap',{extend:Ext.panel.Panel,xtype:'baseMapView',layout:'fit',getMap:function(){return this.map},getMapView:function(){return this.mapView},initMap:function(){var server,type,bingKey,zoom,maxZoom,target;server=Traccar.app.getServer();type=Traccar.app.getPreference('map',null);bingKey=server.get('bingKey');switch(type){case 'custom':server=new ol.layer.Tile({source:new ol.source.XYZ({url:(new DOMParser).parseFromString(server.get('mapUrl'),'text/html').documentElement.textContent,
attributions:''})});break;case 'bingRoad':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'Road'})});break;case 'bingAerial':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'Aerial'})});break;case 'bingHybrid':server=new ol.layer.Tile({source:new ol.source.BingMaps({key:bingKey,imagerySet:'AerialWithLabels'})});break;case 'carto':server=new ol.layer.Tile({source:new ol.source.XYZ({url:'https://cartodb-basemaps-{a-d}.global.ssl.fastly.net/light_all/{z}/{x}/{y}.png',
attributions:'\x26copy; \x3ca href\x3d"http://www.openstreetmap.org/copyright"\x3eOpenStreetMap\x3c/a\x3e contributors, \x26copy; \x3ca href\x3d"https://carto.com/attributions"\x3eCARTO\x3c/a\x3e'})});break;case 'baidu':server=new ol.layer.Tile({source:new ol.source.XYZ({projection:'BD-MC',tileUrlFunction:function(tileCoord){var z=tileCoord[0],x=tileCoord[1];tileCoord=tileCoord[2];var index;index=((x<<z)+tileCoord)%5;0>x&&(x='M'+-x);0>tileCoord&&(tileCoord='M'+-tileCoord);return'http://online{}.map.bdimg.com/onlinelabel/?qt\x3dtile\x26x\x3d{x}\x26y\x3d{y}\x26z\x3d{z}\x26styles\x3dpl'.replace('{}',
...
this.map.on('pointermove',function(e){this.forEachFeatureAtPixel(e.pixel,function(){return!0})?target.style.cursor='pointer':target.style.cursor=''});this.map.on('click',function(e){var features=this.map.getFeaturesAtPixel(e.pixel,{layerFilter:function(layer){return!layer.get('name')}});if(features)for(e=0;e<features.length;e++)this.fireEvent('selectfeature',features[e]);else this.fireEvent('deselectfeature')},this)},listeners:{afterrender:function(){this.initMap()},resize:function(){this.map.updateSize()}}},
function(){proj4.defs('BD-MC','+proj\x3dmerc +lon_0\x3d0 +units\x3dm +ellps\x3dclrk66 +no_defs');proj4.defs('EPSG:3395','+proj\x3dmerc +lon_0\x3d0 +k\x3d1 +x_0\x3d0 +y_0\x3d0 +datum\x3dWGS84 +units\x3dm +no_defs');ol.proj.get('EPSG:3395').setExtent([-2.0037508342789244E7,-2.0037508342789244E7,2.0037508342789244E7,2.0037508342789244E7])});
...
this.getView().getSelectionModel().getSelection()[0];Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedMaintenances,items:{xtype:'linkMaintenancesView',baseObjectName:'userId',linkObjectName:'maintenanceId',storeName:'AllMaintenances',linkStoreName:'Maintenances',baseObject:user.getId()}}).show()},onSelectionChange:function(selection,selected){var disabled=0===selected.length;this.lookupReference('userDevicesButton').setDisabled(disabled);this.lookupReference('userGroupsButton').setDisabled(disabled);
...
this.lookupReference('toolbarCommandsButton').setHidden(Traccar.app.getPreference('limitCommands',!1));this.lookupReference('toolbarMaintenancesButton').setHidden(Traccar.app.getVehicleFeaturesDisabled()||Traccar.app.getBooleanAttributePreference('ui.disableMaintenances'))},onGeofencesClick:function(){var group=this.getView().getSelectionModel().getSelection()[0];Ext.create('Traccar.view.BaseWindow',{title:Strings.sharedGeofences,items:{xtype:'linkGeofencesView',baseObjectName:'groupId',linkObjectName:'geofenceId',
...
Traccar.app.getBooleanAttributePreference('ui.disableDrivers')),this.lookupReference('settingsCommandsButton').setHidden(Traccar.app.getPreference('limitCommands',!1)),this.lookupReference('settingsMaintenancesButton').setHidden(Traccar.app.getVehicleFeaturesDisabled()||Traccar.app.getBooleanAttributePreference('ui.disableMaintenances'))},onUserClick:function(){var dialog=Ext.create('Traccar.view.dialog.User',{selfEdit:!0});dialog.down('form').loadRecord(Traccar.app.getUser());dialog.lookupReference('testNotificationButton').setHidden(!1);
